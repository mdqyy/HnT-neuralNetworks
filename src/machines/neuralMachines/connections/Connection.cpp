...
Connection::Connection() :
		from(0), to(0), weights(cv::Mat()) {
}

Connection::Connection(Layer* _from, Layer* _to, uint _seed) :
		from(_from), to(_to), weights(cv::Mat()) {
...
Connection::Connection(Layer* _from, Layer* _to, cv::Mat _weights) :
	...
	RNG rng;
	initializeWeights(rng.next());
}

Connection::Connection(const Connection& _c) :
		from(0), to(0) {
...
void Connection::setWeights(Mat _weights) {
	if (from != 0 && to != 0) {
		if (to->getNumUnits() == ((uint) weights.rows)
				&& (from->getNumUnits() + 1 == ((uint) weights.cols)
						|| (from->getNumUnits() + 1 + to->getNumUnits() == ((uint) weights.cols) && to->isRecurrent()))) {
			weights = _weights;
		} else {
