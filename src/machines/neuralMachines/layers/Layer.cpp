...
Layer::Layer() :
		Machine("Layer"), numUnits(1), inputConnection(0), outputConnection(0), outputSignal(FeatureVector(2)), deltas(ErrorVector(2)), networkInputSignal(
				FeatureVector(1)), inputSignal(FeatureVector(0)), recurrent(false), inputSignals(std::vector<FeatureVector>()), outputSignals(
				std::vector<FeatureVector>()) {

}

Layer::Layer(uint _numUnits, string _name, bool _recurrent) :
		Machine(_name), numUnits(_numUnits), inputConnection(0), outputConnection(0), outputSignal(FeatureVector(_numUnits + 1)), deltas(
				ErrorVector(_numUnits + 1)), networkInputSignal(FeatureVector(_numUnits)), inputSignal(FeatureVector(0)), recurrent(_recurrent), inputSignals(
				std::vector<FeatureVector>()), outputSignals(std::vector<FeatureVector>()) {
	assert(numUnits>0);
}

Layer::Layer(const Layer& _cl) :
		Machine(_cl.getName()), numUnits(_cl.getNumUnits()), inputConnection(0), outputConnection(0), outputSignal(FeatureVector(numUnits + 1)), deltas(
				ErrorVector(numUnits + 1)), networkInputSignal(FeatureVector(_cl.getNumUnits())), inputSignal(FeatureVector(0)), recurrent(_cl.isRecurrent()), inputSignals(
				std::vector<FeatureVector>()), outputSignals(std::vector<FeatureVector>()) {
...
FeatureVector Layer::createInputSignal() {
	FeatureVector inSig = FeatureVector(0);
	if (isRecurrent()) {
