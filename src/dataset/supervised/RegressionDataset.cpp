...
void RegressionDataset::simpleSave(string _fileName){
  ofstream ofs;
  ofs.open(_fileName.c_str());
  ofs << name << " "<< numSamples << " " << fvLength<< endl;
  for(uint i=0; i<data.size();i++){
    for(uint j=0; j<data[i].size(); j++){
      for(uint k=0; k< data[i][j].getLength();k++){
	ofs << data[i][j][k] << " " ;
...
void RegressionDataset::simpleLoad(string _fileName){
  ifstream ifs;
  ifs.open(_fileName.c_str());
  string inName = "noName";
  uint inNumSamples = 0;
  uint inFvLength = 0;
  ifs >> inName ;
  ifs >> inNumSamples;
  ...
  name = inName;
  realv value = 0;
  for(uint i=0 ; i < inNumSamples ; i++ ){
    vector<FeatureVector> sequence;
    FeatureVector fv(inFvLength);
    for(uint j=0; j<inFvLength; j++){
      ifs >> value;
      fv[j] = value;
    }
    sequence.push_back(fv);
